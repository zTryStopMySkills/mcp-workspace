...
export function ListView({ items, onUpdateItem, onDeleteItem, onMarkSeen }: ListViewProps) {
  if (items.length === 0) return null;

  return (
    <div className="border border-white/8 rounded-xl overflow-hidden">
      {/* Header */}
      <div className="grid grid-cols-[auto_1fr_120px_90px_80px_80px] gap-3 px-4 py-2.5 bg-white/[0.03] border-b border-white/8 text-xs font-medium text-slate-500 uppercase tracking-wider">
        <span className="w-6" />
        <span>Nombre</span>
        <span>Estado</span>
        <span>Fecha</span>
        ...
        {items.map((item, i) => {
          ...
          return (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className={`group grid grid-cols-[auto_1fr_120px_90px_80px_80px] gap-3 items-center px-4 py-3 hover:bg-white/[0.04] transition-colors ${
